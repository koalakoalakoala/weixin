							...
							<div class="dm-item Fix my-order-item">
                            	...
                            	<a <?php  if($model->store && $model->store->ischeck == 4){ ?> onclick="$.MsgBox.Alert('抱歉', '店铺已关闭，商品已下架！');" <?php } ?>  href="<?=$model->store && $model->store->ischeck ==4 ? 'javascript:void(0)' : Url::to(['/goods/goods/view?id='.$v->goods_id])?>">
									<div class="cnt shopcont shopcont_p">
										<div class="shoppr">
											<img class="pic"
												 src="<?=$v->goodsSku->goods->goodsgallery? Yii::$app->params['img_domain'].$v->goodsSku->goods->goodsgallery->image : '' ?>"
												 alt="">
										</div>
											...
											<div class="content-wrap2">
												<div class="shop-content ">
													<div class="title clearfix">
														<span class="in-tit fl zd-width"><?=$v->goodsSku->goods->name?></span>
														<div class="sc-btn fr zdc-top">
															<span class="r-fc">¥<?=$v->goodsSku->market_price + $v->goodsSku->goods->freight?></span>
															<br/>
								...
								<div class=" clearfix">
<!--									<div class="fr"><a href="#" class="my-order-btn">评价</a></div>-->
								</div>
							</div>
						<?php
						$count += $v->number;

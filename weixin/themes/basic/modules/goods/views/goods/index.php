...
<div class="d-commodity-list">
	...
	<!--<div class="d-tab-background">
		<div class="d-tab-title">
			<a class="d-mod-nav-item d-mod-nav-cur" href="#">
				商品
			</a>
			<a class="d-mod-nav-item" href="<?= Url::to(['/store/store/index?category_id='.$category_id]); ?>">
				店铺
			</a>
		</div>
	</div>-->
	...
	<div class="d-tab-background stick-tittle">
		<ul class="d-product-main">
			<li>
				<a href="<?=Url::to(['index?sort='.((isset($_GET['sort']) && $_GET['sort'] == '-sales') ? 'sales' : '-sales').'&category_id_list='.$category_id_list.'&goods_name='.$goods_name.'&store_id='.$store_id])?>" class="sort-btn <?= $font_color_sales ?>">
					<i>销量</i>
			...
			<li>
				<a href="<?=Url::to(['index?sort='.((isset($_GET['sort']) && $_GET['sort'] == '-market_price') ? 'market_price' : '-market_price').'&category_id_list='.$category_id_list.'&goods_name='.$goods_name.'&store_id='.$store_id])?>" class="sort-btn <?= $font_color_price ?>">
					价格
			...
			<li>
				<a href="<?=Url::to(['index?sort='.((isset($_GET['sort']) && $_GET['sort'] == '-shelves_time') ? 'shelves_time' : '-shelves_time').'&category_id_list='.$category_id_list.'&goods_name='.$goods_name.'&store_id='.$store_id])?>" class="sort-btn <?= $font_color_shelves_time ?>">
					上架时间
			...
			<div class="list" >
				<div class="d-sidebar-container d-sidebar-container_p">
					<div class="d-sidebar-main">
						...
						'method' => 'post',
						'id'=>'searchForm',
						'action'=>Url::toRoute('/goods/goods/index')
						]); ?>
						<ul>
						...
						<ul>
							<?= $form->field($searchModel, 'name',['options'=>['class'=>'control-group span8'] ])->label(false)->hiddenInput($options = []); ?>
						</ul>

						<ul>
							<?= $form->field($searchModel, 'category_id_list',['options'=>['class'=>'control-group span8'] ])->label(false)->hiddenInput($options = []); ?>
						</ul>

						<ul>
							<?= $form->field($searchModel, 'goods_category_id',['options'=>['class'=>'control-group span8'] ])->label(false)->hiddenInput($options = []); ?>
						</ul>

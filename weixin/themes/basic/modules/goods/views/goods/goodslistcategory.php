...
<div class="d-commodity-list">
   ...
   <!-- <div class="d-tab-background">
        <div class="d-tab-title">
            <a class="d-mod-nav-item " href="<?= Url::to(['/store/store/view?id='.$store_id]); ?>">
                店铺首页
            </a>
            <a class="d-mod-nav-item" href="<?= Url::to(['/store/store/category?id='.$store_id]); ?>">
                商品分类
            </a>
        </div>
    </div>-->
    ...
    <div class="d-tab-background stick-tittle">
        <ul class="d-product-main">
            <li>
                <a href="<?=Url::to(['goods-list-category?sort='.((isset($_GET['sort']) && $_GET['sort'] == '-shelves_time') ? 'shelves_time' : '-shelves_time').'&price='.$price.'&brand_id='.$brand_id.'&category_id='.$category_id.'&store_id='.$store_id.'&diy_cid='.$diy_cid])?>" class="sort-btn <?= $font_color_shelves_time ?>">
                    综合
            ...
            <li>
                <a href="<?=Url::to(['goods-list-category?sort='.((isset($_GET['sort']) && $_GET['sort'] == '-sales') ? 'sales' : '-sales').'&price='.$price.'&brand_id='.$brand_id.'&category_id='.$category_id.'&store_id='.$store_id.'&diy_cid='.$diy_cid])?>" class="sort-btn <?= $font_color_sales ?>">
                    <i>销量</i>
            ...
            <li>
                <a href="<?=Url::to(['goods-list-category?sort='.((isset($_GET['sort']) && $_GET['sort'] == '-market_price') ? 'market_price' : '-market_price').'&price='.$price.'&brand_id='.$brand_id.'&category_id='.$category_id.'&store_id='.$store_id.'&diy_cid='.$diy_cid])?>" class="sort-btn <?= $font_color_price ?>">
                    价格
                ...
                <div class="d-sidebar-container d-sidebar-container_p">
                    <div class="d-sidebar-main">
                        <?php $form = ActiveForm::begin([
                            'method' => 'post',
                            'id'=>'searchForm',
                            'action'=>Url::toRoute('/goods/goods/goods-list-category')
                        ]); ?>
                        ...
                        <ul>
                            <?= $form->field($searchModel, 'diy_cid',['options'=>['class'=>'control-group span8'] ])->label(false)->hiddenInput($options = []); ?>
                        </ul>
